...
public class BTWRApiEmiPlugin implements EmiPlugin {
    ...
    public static EmiRecipeCategory category(String id, EmiStack icon) {
        return new EmiRecipeCategory(Identifier.of(BTWRApi.MOD_ID, id), icon, icon::render);
    }
    ...
    private void registerCraftingShapelessExtended(EmiRegistry registry) {
        ...
        for (ExtendedShapelessRecipe recipe : getRecipes(registry, BTWRApiRecipes.EXTENDED_SHAPELESS_RECIPE_TYPE)) {
    ...
    private static void addRecipeSafe(EmiRegistry registry, Supplier<EmiRecipe> supplier, Recipe<?> recipe) {
        try {
            ...
            EmiReloadLog.warn("Exception thrown when parsing vanilla a" + BTWRApi.MOD_ID + "recipe" + EmiPort.getId(recipe));
